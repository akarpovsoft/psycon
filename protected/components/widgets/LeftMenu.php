...
class LeftMenu extends CWidget {
    ...
    public function run() {
        if ($this->guest) {
            ...
            if($this->user_type == 'gift_chat_pending')
        	   ...
        	   array('title'=>Yii::t('lang','History'),
        	         'link'=>Yii::app()->params['http_addr'].'history',   ///'/'.PsyConstants::getName(PsyConstants::PAGE_CHAT),
        	         'class'=>'navigation',),
        	   array('title'=>Yii::t('lang','Contact_support'),
        	         'link'=>Yii::app()->params['http_addr'].'site/contact', ///'/'.PsyConstants::getName(PsyConstants::PAGE_CONTACT_US),
        	         'class'=>'navigation',),
        ...
        if($this->layout_type)
            $this->render('leftMenu',array('links'=>$links, 'leftArt' => $this->articles, 'layout' => $this->layout_type));
        else $this->render('leftMenu',array('links'=>$links, 'leftArt' => $this->articles, 'layout' => 'default'));
    }
}
?>