...
class TimeEvents extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('session_id, event_flag', 'required'),
			array('session_id', 'length', 'max'=>50),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, session_id, event_flag', 'safe', 'on'=>'search'),
		);
	}
	...
	private function setFlag($event_flag)
	{
		...
		$TimeEvents->session_id = $this->session_id;
		$TimeEvents->save();
    ...
    {
    	$TimeEvents = $this->getEventFlag($this->session_id);
		$flagarr = array();
		if(is_object($TimeEvents))
		{
			$flagarr = unserialize($TimeEvents->event_flag);
    ...
    {
    	$this->session_id = $chatContext->session_id;
