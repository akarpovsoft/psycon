<html>
<head>
<link rel="stylesheet" type="text/css" href="<?php echo ChatHelper::baseUrl(); ?>css/style.css" />
<script type="text/javascript" src="<?php echo ChatHelper::baseUrl(); ?>js/jquery-1.5.2.js"></script>
<script type="text/javascript" src="<?php echo ChatHelper::baseUrl(); ?>js/jquery.json-2.2.js"></script>
<SCRIPT LANGUAGE="JavaScript">
	...
	//alert('ok');
	    clearTimeout(timeoutHandler);
            window.location.href="<?php echo ChatHelper::moduleUrl(); ?>monitor?rld=1";
    }
    ...
    {            
        ...
        <?php if($active->chat_type == 7): ?>
            window.location.href = "<?php echo ChatHelper::moduleUrl(); ?>monitor/startChatting?request_id="+session_id;
        <?php else: ?>
            var date = new Date();
    ...
    function checkForUpdates()
	{
		runAjax({			
			...
			url     : "<?php echo ChatHelper::moduleUrl(); ?>monitor/checkNewClient",			
			...
			error	: function(XMLHttpRequest, errorType)
			{
				...
				if(errorsCounter == 3){
					alert('Connection lost.Either internet connection is lost or server is unavailable'+errorType+XMLHttpRequest.responseText);
    				errorsCounter = 0;
				}
				timeoutHandler = setTimeout("checkForUpdates()", 3000);
			} 
    ...
    function changeReaderStatus(status, click)
	{
            if(typeof click != 'undefined'){
                document.getElementById('spinner').innerHTML = '<img src="<?php echo ChatHelper::baseUrl(); ?>images/loading_21_21.gif" align="right">';
            }
		...
		runAjax({
			...
			url     : "<?php echo ChatHelper::moduleUrl(); ?>monitor/readerStatusChange",
			success : function () {},
			error : function(XMLHttpRequest, errorType)
			{
				try {
...
<a href="<?php echo ChatHelper::moduleUrl(); ?>/monitor/logout"><?php echo Yii::t('lang', 'Log_Off'); ?></a><br>
