...
timer=window.setTimeout("refresh();", 100);
function refresh(){
	parent.document.location.href = "http://www.psychic-contact.com/advanced/chat.php";
}
</SCRIPT>
<?php
}
...
//DrawNavigationBar($STR_CHAT, array($PageName => "chatstart.php"), false);
...
//echo "SELECT $tableOperators.rr_record_id,$tableOperators.name FROM $tableOperators left join $tableChatSessions on ($tableOperators.rr_record_id=$tableChatSessions.reader_id and $tableChatSessions.type='reader')";
...
<b><font color="#FF0000">*WARNING* Disable POPUP blockers or your chat will not start!<br>
<br>
If you experience problems accessing the chatroom. Please close and re-open your browser.
If it persists- please try our "ER" (Emergency ChatRoom) 
</font></b><br><br>
<b>
FREE 10MINS FIRST TIME CHATTERS:<br />
PLEASE NOTE:</b> You may leave the chat at anytime you like {BEFORE THE 10 FREE MINS ARE USED UP} and any unused FREE time will remain in your account for <b>7 days</b> at which point it will be canceled and no charge will be incurred for the un-used time).<b> You may use your free 10mins with up to 2 Readers -at your discretion</b> within the 7 days 
<br><br>
...
<input type=hidden name=account_id value="<?=$login_account_id?>">
        ...
        <tr>
        	<td colspan="3">
			<input name="nopopup_debug" type="checkbox" value="yes"> <font color="#FF0000">[Emergency Chat Room] Use only if the regular chat room is failing.</font>
